...
function CharacterDetailWindow:CreateExpansionSection(expName, expData, profName, yOffset, xOffset)
    ...
    if hasKnowledgeSystem then
        -- Crafting Order (skip if completed)
        if weekly.craftingOrderQuest ~= true then
            ...
            local orderStatusIcon = "|TInterface\\RaidFrame\\ReadyCheck-NotReady:12:12|t"
        ...
        if expData.missingOneTimeTreasures and #expData.missingOneTimeTreasures > 0 then
            ...
            oneTimeHeader:SetText(string.format("|cffff8800One-Time (%d):|r", #expData.missingOneTimeTreasures))
            oneTimeHeader:SetTextColor(1, 0.53, 0, 1)
            yOffset = yOffset - 14
